...
class Netzarbeiter_GroupsCatalog2_Model_Observer
{
    ...
    public function catalogProductCollectionLoadBefore(Varien_Event_Observer $observer)
    {
	    ...
	    //Used as a flag to trigger specific query where clauses
		$type = 'product';

        $this->_addGroupsCatalogFilterToCollection($collection, $type);
    }
    ...
    public function wishlistItemCollectionLoadBefore(Varien_Event_Observer $observer)
    {
        ...
        if ($helper->isModuleActive() && !$helper->isDisabledOnCurrentRoute()) {
            ...
            $this->_getResource()
                    ->addGroupsCatalogFilterToWishlistItemCollection($collection, $customerGroupId, $storeId, $type);
        }
    }
     ...
     * @param string $type Used as a flag to trigger inventory and date where clause.  Custom 4/11/2017
     * @return void
     */
    protected function _addGroupsCatalogFilterToCollection(Varien_Data_Collection_Db $collection, $type = 'Default')
    {
        $helper = $this->_getHelper();
        if ($helper->isModuleActive() && !$helper->isDisabledOnCurrentRoute()) {
            $customerGroupId = $helper->getCustomerGroupId();
            ...
            $this->_getResource()
                    ->addGroupsCatalogFilterToCollection($collection, $customerGroupId, $type);
        }
    }
