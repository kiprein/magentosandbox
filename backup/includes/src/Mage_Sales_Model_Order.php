...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    /**
     * Added by Mark Wickline 9/16/19.
     * A function for order emails to format custom 3rd party input information
     */
    public function formatThirdParty(){
        if($this->getShippingMethod(true)->getMethod() != 'modalcart_fedex_3rdparty') //mgw_modalcart_fedex_3rdparty
            return '';
        $data = unserialize($this->getData('3rdpartyFedex'));
        ...
        $build = $data['acc'] . "<br/>";
        $build .= $data['carrier'] ? $data['carrier'] . "<br/>" : '';
        $build .= $data['company'] ? $data['company'] . "<br/>" : '';
        $build .= "${city}, ${state}, ${zip} <br/>";
        $build .= $data['country'] . "<br/>";
        $build .= "Preferred Method: " . $data['preferred'] . "<br/>";
        return $build;
    }
