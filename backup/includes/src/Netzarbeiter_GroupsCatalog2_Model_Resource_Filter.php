    ...
    extends Mage_Core_Model_Resource_Db_Abstract
{
     ...
     * @param string $type Used as a flag to trigger specific query where clauses - JS
     * @return void
     */
    public function addGroupsCatalogFilterToCollection(Varien_Data_Collection_Db $collection, $groupId, $type = 'Default')
    {
        ...
        if ($this->_doesIndexExists()) {
            ...
            $this->_addGroupsCatalogFilterToSelect(
                $collection->getSelect(), $filterTable, $groupId, $collection->getStoreId(), $entityIdField, $type
            );
        }
    }
    ...
    public function addGroupsCatalogFilterToWishlistItemCollection(
        Mage_Wishlist_Model_Resource_Item_Collection $collection, $groupId, $storeId, $type = 'Default'
    )
    {
        $select = $collection->getSelect();
        $entityField = 'main_table.product_id';
        $this->addGroupsCatalogProductFilterToSelect($select, $groupId, $storeId, $entityField, $type);
    }
    ...
    public function addGroupsCatalogProductFilterToSelect(
        Zend_Db_Select $select, $groupId, $storeId, $entityField = null, $type = 'Default'
    )
    {
        $this->_addGroupsCatalogEntityFilterToSelect(
            Mage_Catalog_Model_Product::ENTITY, $select, $groupId, $storeId, $entityField, $type
        );
    }
    ...
    protected function _addGroupsCatalogEntityFilterToSelect(
        $entityType, Zend_Db_Select $select, $groupId, $storeId, $entityField = null, $type = 'Default'
    )
    {
        /* @var $helper Netzarbeiter_GroupsCatalog2_Helper_Data */
        $helper = Mage::helper('netzarbeiter_groupscatalog2');
        ...
        if ($this->_doesIndexExists()) {
            ...
            $this->_addGroupsCatalogFilterToSelect($select, $table, $groupId, $storeId, $entityField, $type);
        }
    }
    ...
    protected function _addGroupsCatalogFilterToSelect(
        Zend_Db_Select $select, $table, $groupId, $storeId, $entityField = null, $type = 'Default'
    )
    {
        if (!$entityField) {
        ...
        */
		...
		/**
		 * Check the query type and if inventory and date were used
		 */
	    $inventory = Mage::app()->getRequest()->getParam('inventory');
	    $before = Mage::app()->getRequest()->getParam('available_before');
	    $where = $select->getPart(Zend_Db_Select::WHERE);

		if($type == 'product' && ($inventory || $before)) {
			//There is a weird issue where sometimes there where clause is empty
			if(count($where) >= 1) {
				$where[] = "AND 1=1";
			} else {
				$where[] = "1=1";
			...
			$inventoryTables = 'inventory_avail';
			$onClause = "{$inventoryTables}.product_id={$entityField}";

			//Need to build the where statement based on what was selected
			if($inventory) {
				$where[] = "AND {$inventoryTables}.qty >= {$inventory}";
			}

			$date = date('Y-m-d');
			if($before) {
				$date = date('Y-m-d', strtotime($before));
			}
			$where[] = "AND {$inventoryTables}.date <= '$date'";

			//Try this instead once table is populating
//			if ($where && substr($where[0], 0, 3) == 'AND'){
//				$where[0] = substr($where[0], 3);
//			}
			...
			$select->columns('MAX(date) AS available_before')->group(array('e.entity_id'));

			//log the query for testing
			//Mage::log( var_export( $select->__toString() , true));
		}
