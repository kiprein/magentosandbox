...
class Image_Sync extends Mage_Shell_Abstract {
	...
	public function run() {
		...
		//	"SELECT DISTINCT(a.i_owner_id) AS erp_id, b.s_item_num 
		//		FROM t_image AS a 
		//		INNER JOIN t_product AS b on a.i_owner_id = b.k_product_id 
		//		WHERE a.dt_updated > date_sub(now(), interval 30 minute)
                //" );

		// No, update all the products. There may be a product that USED TO HAVE a packaing image but doens't now.
		$productInformation = $read->query(
			"SELECT s_item_num
                         FROM t_product
                         WHERE b_offline = 0
		...
		foreach ($productInformation as $productInfo) {
			...
			echo "IMAGE: " . $this->getGiftBoxImage($read, $productInfo['s_item_num']) . "\n";;
			echo "M Id: " . $productId . "\r\n\r\n";
			...
			if ( $productId ) {
				...
				$product
					->setGiftBoxImage( $this->getGiftBoxImage( $read, $productInfo['s_item_num'] ) );
