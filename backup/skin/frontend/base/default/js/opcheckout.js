...
Checkout.prototype = {
    initialize: function(accordion, urls){
        ...
        this.steps = ['login', 'shipping', 'shipping_method', 'billing', 'payment', 'review']; //#mgwAddingStep
        //We use billing as beginning step since progress bar tracks from billing
        this.currentStep = 'shipping';

        this.accordion.sections.each(function(section) {
            Event.observe($(section).down('.step-title'), 'click', function(){
                this._onSectionClick.bindAsEventListener(this);
                document.fire('mgw:adjustHeightToDiv');
            }.bind(this));
        }.bind(this));
...
//        $('billing:use_for_shipping_yes').checked = flag;
    ...
    /**
     * Written by Mark wickline 9/11/19.
     * The order of checkout processes was changed. There need to be a way to update
     * the billing form to match the shipping form based on a click. This only happens
     * if a customer clicks the next button on the shipping step.
     */
    useForBilling: function (){
        if($('shipping-useForBilling').checked){
            arrElements = Form.getElements(this.form);
            for (var elemIndex in arrElements) {
                if (arrElements[elemIndex].id) {
                    var destinField = $(arrElements[elemIndex].id.replace(/^shipping:/, 'billing:'));
                    if (destinField){
                        destinField.value = arrElements[elemIndex].value;
                    }
                }
            }
            billingRegionUpdater.update();
    ...
    validate: function() {
        ...
        for (var i=0; i<methods.length; i++) {
            if (methods[i].checked) {
                /**
                 * #MGW 9/12/19 added validation for 3rd party shipping information.
                 */
                if(methods[i].value == 'mgw_modalcart_fedex_3rdparty'){
                    var requiredFields = jQuery('.thirdParty-required');
                    for( var x = 0; x < requiredFields.length; x ++){
                        if( requiredFields[x].value == ''){
                            alert(requiredFields[x].title + ' is required!');
                            return false;
                        }
                    }
                }
...
Review.prototype = {
    initialize: function(saveUrl, successUrl, agreementsForm, reviewForm){
        this.saveUrl = saveUrl;
    ...
    save: function(){
        ...
        /**
         * MGW need to add reivewForm to params to be able to save
         * additional order notes textarea to database.
         */
        if(this.reviewForm){
            params += '&'+Form.serialize(this.reviewForm);
        }
        params.save = true;
        var request = new Ajax.Request(
            this.saveUrl,
...
//MGW added step in checkout process
