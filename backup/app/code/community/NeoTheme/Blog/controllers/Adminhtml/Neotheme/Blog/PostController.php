...
class NeoTheme_Blog_Adminhtml_Neotheme_Blog_PostController extends NeoTheme_Blog_Controller_Adminhtml_Abstract {
    ...
    public function saveAction() {
		...
		try{
			...
			if((bool)$data['blog_image']['delete']==1) {
				...
				if (isset($_FILES)){
					...
					if ($_FILES['blog_image']['name']) {
						...
						if($this->getRequest()->getParam("id")){
							$model_img = Mage::getModel("neotheme_blog/post")->load($this->getRequest()->getParam("id"));
							if($model_img->getData('blog_image')){
									$io = new Varien_Io_File();
									$io->rm(Mage::getBaseDir('media').DS.implode(DS,explode('/',$model_img->getData('blog_image'))));
							}
						}
									$path = Mage::getBaseDir('media') . DS . 'neotheme_blog' . DS .'neotheme_blog'.DS;
									$uploader = new Varien_File_Uploader('blog_image');
									$uploader->setAllowedExtensions(array('jpg','png','gif'));
									$uploader->setAllowRenameFiles(false);
									$uploader->setFilesDispersion(false);
									$destFile = $path.$_FILES['blog_image']['name'];
									$filename = $uploader->getNewFileName($destFile);
									$uploader->save($path, $filename);
			
									$img='neotheme_blog/neotheme_blog/'.$filename;
					}
				}
			}
