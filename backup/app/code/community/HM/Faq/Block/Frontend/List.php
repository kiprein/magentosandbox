...
class HM_Faq_Block_Frontend_List extends Mage_Core_Block_Template
{
	 ...
	 */
	public function getItemCollectionByCategory($category)
	{	
		
		$resource = Mage::getSingleton('core/resource'); 
		$readConnection = $resource->getConnection('core_read'); 
		$query = 'select * from faq where faq_id IN(SELECT faq_id FROM faq_category_item where category_id='.$category.') and is_active=1 order by item_sort_order ASC';
