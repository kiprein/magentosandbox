...
class Amasty_Shopby_Helper_Attributes extends Amasty_Shopby_Helper_Cached
{
     ...
     * @return bool
     */
    public function isMapped($attribute_id)
    {
        if ($this->mappedAttributeIds === null) {
            $this->mappedAttributeIds = Mage::getModel('amshopby/filter')
                ->getCollection()
                ->addFieldToFilter('use_mapping', 1)
                ->getAllAttributeIds();
        }

        return in_array($attribute_id, $this->mappedAttributeIds);
    }
